...
impl<'a> Parser<'a> {
    ...
    fn parse_statement(&mut self) -> Option<Box<dyn ast::Statement>> {
        match self.curr_token.kind {
            TokenKind::LET => {
                if let Some(letst) = self.parse_let_statement() {
                    return Some(letst as Box<dyn ast::Statement>);
                }
                None
            }
            _ => None,
...
mod test_parser {
    ...
    fn check_parse_errors(p: &mut Parser) {
        ...
        for (_, err_msg) in errors.iter().enumerate() {
